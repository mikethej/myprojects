...
class DX_Auth
{
	...
	*/
	...
	function register($username, $password, $email,$nome_completo,$data_nascimento)
	{		
		// Load Models
		$this->ci->load->model('dx_auth/users', 'users');
		$this->ci->load->model('dx_auth/user_temp', 'user_temp');
		...
		$new_user = array(			
			'username'				=> $username,
			//'password' 					=> crypt($password,password_hash($password, PASSWORD_DEFAULT)),	
			...
			'last_ip'					=> $this->ci->input->ip_address(),
                        ...
                        'au_part_reg_ins_user'                          => FALSE,
                        'au_part_reg_clinicos'                          => FALSE
		);
