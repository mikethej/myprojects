...
<head>
    ...
    <title>Licitaç&otilde;es autom&aacute;ticas | Tuga-Buy</title>
	    	...
	    	<div class="row">    		
	    		<div class="col-sm-12">
                    <?php
                        ...
                        $sqlo = "SELECT * FROM utilizadores WHERE nick = '".$nick."';";
                        $queryo = mysqli_query($conn, $sqlo) or die("Error2");
                        while($row = mysqli_fetch_assoc($queryo)){
                            $idp = $row['user_id'];
                        }
                        echo "<form action= '' class='center' method='post'>
                            <table>
                                <tr>
                                    <td>Incrementador: </td>
                                    <td><input type='number' name='intervalo' step='0.01' value='0.00' min='0.00'></td>
                                </tr>
                                <tr>
                                    <td>Valor m&aacute;ximo: </td>
                                    <td><input type='number' name='maximo' step='0.01' value='".$vatual."'></td>
                                </tr>
                                <tr>
                        ...
                        if($submit == 'Guardar'){
                            ...
                            $maximo = $_POST['maximo'];
                            $select = "SELECT * FROM auto WHERE item_id = '".$idi."' AND user_id = '".$idp."';";
                            $qselect = mysqli_query($conn, $select) or die("Error5");
                            $row = mysqli_fetch_array($qselect);
                            if(empty($row)){
                                $cena="INSERT INTO auto VALUES('$idp', '$idi', '$intervalo', '$maximo')"; 
                                $que = mysqli_query($conn, $cena) or die(mysqli_error($conn));
                                echo "<script>window.location.href = 'perfil';</script>";
                            }
                            else{
                                $cena="UPDATE auto SET increm = '$intervalo', maximo = '$maximo' WHERE item_id='$idi' AND user_id = '$idp';";
                                $que = mysqli_query($conn, $cena) or die("Error7");
                                echo "<script>window.location.href = 'perfil';</script>";
                            }
                        }
                    ?>
