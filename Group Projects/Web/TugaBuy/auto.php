	    		...
	    		<div class="col-sm-12">
                    <div class="show">
                        <?php
                            ...
                            $query = mysqli_query($conn, $sql) or die("Error5");
                            $cenasdavida = mysqli_fetch_all($query);
                            foreach($cenasdavida as $arr => $raw){
				                $idi = $raw[1];
                                $idp = $raw[0];
                                $intervalo = $raw[2];
                                $maximo = $raw[3];
                                ...
                                if(!empty($queryp)){
                                    while($row = mysqli_fetch_assoc($queryp)){
                                        $valorl = $row['valor'];                                            
                                        array_push($riw, $valorl);
                                    }
                                    foreach ($riw as $value){
                                        if($value > $vatual){
                                            $vatual=$value;
					                   }
                                    }
                                ...
                                $queryi = mysqli_query($conn, $sqli) or die("Error1");
                                while($row = mysqli_fetch_assoc($queryi)){
                                    $idpe = $row['user_id'];
                                }
                                if($idp != $idpe && $maximo > $vatual){
                                    $licitacao = $vatual + $intervalo;
                                    $cena="INSERT INTO licitacoes VALUES('$idp', '$idi', '$data_lici', '$licitacao', '0')";
                                    $que = mysqli_query($conn, $cena) or die("Error2");
                                }
                            }
                        ?>
                    </div>
