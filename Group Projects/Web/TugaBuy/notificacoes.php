...
<head>
	...
	<title>Ver notificaç&atilde;o | Tuga-Buy</title>
...
<body>
	...
	<section>
	<div class="container">
		<p id="titulo">Notificaç&otilde;es</p>
		<div class="row">
						...
						<div class="row">
							<div class="col-lg-12">
                                <?php
                                    ...
                                    $sqlxx = "SELECT * FROM notificacoes WHERE item_id='".$idi."';";
                                    ...
                                    while($row = mysqli_fetch_assoc($queryxx)){
                                        ...
                                        $sql = "SELECT * FROM utilizadores WHERE user_id = '".$comprador."';";
                                        $query = mysqli_query($conn, $sql) or die("Error2");
                                        while($row = mysqli_fetch_assoc($query)){
                                            $nick_comprador = $row['nick'];
                                        }
                                        $sql = "SELECT * FROM utilizadores WHERE user_id = '".$vendedor."';";
                                        $query = mysqli_query($conn, $sql) or die("Error3");
                                        while($row = mysqli_fetch_assoc($query)){
                                            $nick_vendedor = $row['nick'];
                                        }
                                        $sqlv = "SELECT * FROM artigos WHERE item_id = '".$idi."';";
                                        $querv = mysqli_query($conn, $sqlv) or die("Error4");
                                        while($row = mysqli_fetch_assoc($querv)){
                                            $nomeart = $row['designacao'];
                                        }
                                        if($idp == $comprador){
                                            echo "<p style='border:1px solid #d3d3d3; padding:10px;'>O artigo ".$nomeart." agora &eacute; seu! Venceu o leil&atilde;o com o preço de €".$valor.", vendido pelo utilizador ".$nick_vendedor.".</p>";
                                            $naosei="UPDATE notificacoes SET comprador_check = 1 WHERE item_id = '$idi'";
                                            $fartaquerys = mysqli_query($conn, $naosei) or die("Error5");                                  
                                        }
                                        if($idp == $vendedor){
                                            echo "<p style='border:1px solid #d3d3d3; padding:10px;'>Comprou o artigo ".$nomeart." foi comprado pelo utilizador ".$nick_vendedor." pelo preço de €".$valor.".</p>";
                                            $naosei="UPDATE notificacoes SET vendedor_check = 1 WHERE item_id = '$idi'";
                                            $fartaquerys = mysqli_query($conn, $naosei) or die("Error5");
                                        }
                                        
                                    }
                                    echo"<a href='perfil'><button type='submit' value='Submit' name='submit' class='btn btn-default' id='posicao'>Voltar para o Perfil</button></a>";
