...
<head>
    ...
    <title>Licitacao | Tuga-Buy</title>
	    	...
	    	<div class="row">    		
	    		<div class="col-sm-12">
                    <?php
                        include 'connect.php';
                        $idi = $_GET['id'];
                        $licitacao = htmlspecialchars($_POST['lici']);
                        $anon = $_POST['anon'];
                        $data = date("Y-m-d");
                        $hora = date("H:i:s");
                        $data_lici = date("Y-m-d H:i:s");

                        if ($_COOKIE['login'] == 'false'){ //Verifica se tem a sessao iniciada
                            echo "<p>Ainda n&atilde;o tem sessão iniciada</p>
                            <p><a href='login'>Inicie sessão aqui</a></p>";
                        }
                        else{
                            $sql = "SELECT * FROM artigos WHERE item_id = '".$idi."';"; //Procura tudo sobre o artigo
                            ...
                            if(strtotime($data)<strtotime($datafim) || (strtotime($data) == strtotime($datafim) && strtotime($hora) < strtotime($horafim))){ //Verifica se o leilao ja acabou
                                $sqlp = "SELECT * FROM licitacoes WHERE item_id = '".$idi."';"; //Procura licitacoes do artigo
                                ...
                                $nickl = $_COOKIE['nick']; //vai ver quem e a pessoa que tem a sessao iniciada
                                $qnick = "SELECT * FROM utilizadores WHERE nick = '$nickl'"; //Procura o utilizador que tem a sessao iniciada
                                $qu = mysqli_query($conn, $qnick) or die("Error");
                                while($row = mysqli_fetch_assoc($qu)){
                                    $idp = $row['user_id'];
                                }
                                if ($licitacao > $vatual){ //Ver se a licitacao e maior que a maior licitacao
                                    if($licitacao <= $mvalor){ //Ver se a licitacao e menor que o limite
                                        $cena="INSERT INTO licitacoes VALUES('$idp', '$idi', '$data_lici', '$licitacao', '$anon')"; //Insere na tabela licitacoes a nova licitacao
                                        $que = mysqli_query($conn, $cena) or die("Error");
                                        echo "<script>window.location.href = 'product-details?id=".$idi."';</script>";
                                    }
                                    else{
                                        if ($vatual > $mvalor){
                                            echo "O limite de preço deste artigo j&aacute; foi ultrapassado.";
                                            $sqk="UPDATE artigos SET data_fim='$data', hora_final='$hora' WHERE item_id='$idi';"; //Insere na tabela licitacoes a nova licitacao
                                            $kerry = mysqli_query($conn, $sqk) or die("Error");
                                        }
                                         else{
                                            echo "O valor que p&ocirc;s &eacute; maior que o limite de preço do artigo.";
                                        }
                                        echo "<a href='product-details?id=".$idi."'>Volte para o artigo</a>";
                                    }
                                }
                                else{
                                    echo "O valor que inseriu &eacute; menor que a maior licitaç&atilde;o";
                                    echo "<a href='product-details?id=".$idi."'>Volte para o artigo</a>";
                                }
                            }
                            else{
                                echo "O prazo para licitar j&aacute; encerrou!";
                            } 
                        }
                    ?>
                </div>
