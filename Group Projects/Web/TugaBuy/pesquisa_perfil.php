                     ...
                     <?php
                        ...
                        if($filtro == 'nome'){
                            ...
                            if($filtro == 'descricao'){
                                ...
                                if($filtro == 'categoria'){
                                    $filter = $_POST['cat'];
                                    $sql = "SELECT * FROM palavraschave WHERE pal_chave='".$filter."';";
                                    $resa = mysqli_query($conn, $sql) or die("Erro aqui1");
                                    while($row = mysqli_fetch_assoc($resa)){
                                        $idi = $row['item_id'];
                                    }
                                    $sqli = "SELECT * FROM artigos WHERE item_id='".$idi."';";
                                    $res = mysqli_query($conn, $sqli) or die("Erro aqui2");	
                                    while($row = mysqli_fetch_array($res, MYSQL_NUM)){
                                        echo "<tr><td>".implode("</td><td><a href='product-details?id=".$idi."'>", $row)."</td><tr>";
                                    }
                                    echo "</table><br>";
                                    ...
                                    if($filtro == 'dataf'){
                                        ...
                                        if($filtro == 'base'){
                                            ...
                                            if($filtro == 'base'){
                                                ...
                                                $resa = mysqli_query($conn, $sqli) or die("Erro aqui");
                                                while($row = mysqli_fetch_assoc($resa)){
                                                    $idp = $row['user_id'];
                                                }
