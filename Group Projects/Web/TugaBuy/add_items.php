					...
					<div class="col-sm-9">
						...
						<div class="mainmenu pull-left">
							...
							<ul class="nav navbar-nav collapse navbar-collapse">
								<li><a href="index.php" class="active">Ofertas do dia</a></li>
							</ul>
						</div>
					</div>
					<div class="col-sm-3">
						...
						<div class="row">
							<div class="col-lg-12">
                                <?php
                                    ...
                                    if ($nomeart&&$descricao&&$valor&&$pchave&&$datafinal&&$datainicial&&$mvalor){
                                        $sql = "SELECT * FROM utilizadores WHERE nick = '".$nick."';";
                                        $quer = mysqli_query($conn, $sql) or die("Error");
                                        while($row = mysqli_fetch_assoc($quer)){
                                            $idp = $row['user_id'];
                                        }
                                        $query = "INSERT INTO artigos VALUES('', '$nomeart', '$idp', '$valor', '$descricao', '$datainicial', '$datafinal', '$hours', '', '$mvalor');";
                                        $resa = mysqli_query($conn, $query) or die(mysqli_error($conn));
                                        if(!$resa){
                                            die("Error: ".$query);
                                        }
                                        $sql_q = "SELECT * FROM artigos WHERE designacao = '".$nomeart."';";
                                        $quera = mysqli_query($conn, $sql_q) or die("Error");
                                        ...
                                        if(isset($_FILES['image'])){
                                            ...
                                            if(empty($errors)==true){
                                                move_uploaded_file($file_tmp,"uploads/".$idi.".jpg");
                                            }
                                            else{
                                                echo "Nope";
                                                print_r($errors);
                                        ...
                                        echo "<p><a href = 'items'>Volte para a p&aacute;gina para adicionar um novo item.</a></p>";
                                    }
                                    ?>
                            </div>
						</div>
